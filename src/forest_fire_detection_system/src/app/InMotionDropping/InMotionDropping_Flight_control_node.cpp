...
*/
    ...
    cout << "please select case (all cases are for single fire point): " << std::endl
         ...
         << "[e] Multiple fire spots multiple drops" << std::endl;
...
*/
    ...
    if (scenario == 'e'){
        //Set UAV destinations
            ...
            //Define callback function for fire spots
            void FireCallback(const geometry_msgs::PoseArrayConstPtr &fire_spots_GPS) {
                // Print number of fire spots
                LOG(INFO) << "The number of fire spots: " << fire_spots_GPS->poses.size() << ".";
                ...
                nodes[0].id = 0;
                nodes[0].x = homeGPos.latitude;
                nodes[0].y = homeGPos.longitude;
                nodes[0].z = homeGPos.altitude;
                ...
                //Define functions to use dynamic programming algorithm to generate the shortest path
                    M = 1 << (N - 1);
                    // ���涥��i��״̬s���ص���ʼ�����С����
                    double dp[N][M];
                    // store the path
                ...
                //Call above dynamic programming functions to get the shortest path
                deriveDis();
                TSP();
                getPath();

                // Put the sorted nodes in array into predefined vector
                for (int i = 1; i < N; i++) {
                    nodes_vec.push_back(nodes[path[i]]);
                }
            }

            //Get fire GPS position
            ros::Subscriber fire_spots_GPS_sub = nh.subscribe("/position/fire_spots_GPS", 1, FireCallback);
